...
def ask_about_resume_second(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_second_text.value)
...
def ask_name(message):
    try:
        ...
        dbworker.set_state(message.chat.id, config.States.S_ENTER_surname.value)
...
def between_name_and_birthday(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_birthday.value)
...
def ask_town(message):
    try:
        ...
        dbworker.set_state(message.chat.id, config.States.S_ENTER_where_are_you_from.value)
    except Exception:
...
def choose_district(message):
    try:
        ...
        dbworker.set_state(message.chat.id, config.States.S_ENTER_district.value)
    except Exception:
        msg = bot.reply_to(message, 'Неверные данные!')
        bot.register_next_step_handler(msg, ask_town)
...
def between_ask_town_and_ask_town_and_district(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_town_and_district.value)
...
def education_1(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_edu.value)
...
def uzb_language(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_uzb.value)
...
def rus_language(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_ru.value)
...
def english_language(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_eng.value)
...
def about_work(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_work_experience.value)
...
def say_experience(message):
    ...
    dbworker.set_state(message.chat.id, config.States.S_ENTER_work_experience_yes.value)
